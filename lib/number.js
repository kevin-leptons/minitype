...
class UInt16 {
    ...
    static fromBuffer(value) {
        ...
        if (value.length > 2) {
            return Result.typeError('overflow buffer 2 bytes')
        }
        let heximal = value.toString('hex')
        let prefixedHeximal = (heximal.length === 0)
            ? '0x0'
            : '0x' + heximal
        let number = Number(prefixedHeximal)
        if (number <= 0) {
            return Result.typeError('expect buffer as unsigned integer number')
        }
        let instance = new UInt16(number)
        return Result.ok(instance)
    }
...
class UInt64 {
    ...
    static fromBuffer(value) {
        ...
        if (value.length > 8) {
            return Result.typeError('overflow buffer 8 bytes')
        }
        let heximal = value.toString('hex')
        let prefixedHeximal = (heximal.length === 0)
            ? '0x0'
            : '0x' + heximal
        let number = BigInt(prefixedHeximal)
        if (number <= 0) {
            return Result.typeError('expect buffer as unsigned integer number')
        }
        let instance = new UInt64(number)
